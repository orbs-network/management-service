...
import { renderVirtualChainManagement, renderHistoricVirtualChainManagement } from './render-vc';
...
test.serial('[integration] getVirtualChainManagement responds according to Ethereum state', async (t) => {
  ...
  t.assert(res.PageStartRefTime > 0);
...
test.serial('[integration] renderHistoricVirtualChainManagement responds according to Ethereum state', async (t) => {
  t.timeout(5 * 60 * 1000);
  ...
  // process
  const res = renderHistoricVirtualChainManagement(1000000, 1300000000, state.getCurrentSnapshot(), config);
